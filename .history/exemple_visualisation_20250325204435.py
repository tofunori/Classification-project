...
def main():
    ...
    if args.validation and not os.path.isfile(args.validation):
        print(f"ERREUR: Le fichier de validation {args.validation} n'existe pas.")
        sys.exit(1)
    ...
    if args.validation:
        cmd += f" --validation \"{args.validation}\""
        
    if args.classe_colonne and args.classe_colonne != 'classvalue':
        cmd += f" --classe_colonne \"{args.classe_colonne}\""
