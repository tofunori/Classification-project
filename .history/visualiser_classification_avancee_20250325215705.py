...
def genere_analyse_pca(classes_info):
    ...
    try:
        ...
        # Utiliser exactement la même méthode que pour la classification supervisée
        ...
        # Ajuster les échelles des axes pour qu'elles soient cohérentes avec la classification supervisée
        # Souvent, on veut que les axes aient la même échelle pour une meilleure visualisation
        x_lim = max(abs(np.min(pca_result[:, 0])), abs(np.max(pca_result[:, 0])))
        y_lim = max(abs(np.min(pca_result[:, 1])), abs(np.max(pca_result[:, 1])))
        ...
        if pca_result.shape[1] >= 3:
            ...
            # Ajuster les échelles pour être cohérentes
            z_lim = max(abs(np.min(pca_result[:, 2])), abs(np.max(pca_result[:, 2])))
            limit_3d = max(x_lim, y_lim, z_lim) * 1.1
            
            ax2.set_xlim(-limit_3d, limit_3d)
            ax2.set_ylim(-limit_3d, limit_3d)
            ax2.set_zlim(-limit_3d, limit_3d)
        ...
        # Afficher les coefficients absolus pour mieux visualiser l'importance
        components_abs = np.abs(components)
        
        # Créer un heatmap avec une échelle de couleurs symétrique
        cmap = sns.diverging_palette(220, 10, as_cmap=True)
        
        sns.heatmap(
            components, 
            annot=True, 
            fmt='.2f',  # Limiter à 2 décimales pour la clarté 
            cmap='coolwarm',
            center=0,  # Centrer à 0 pour une échelle symétrique
            xticklabels=columns,
            yticklabels=[f'PC{i+1} ({var*100:.1f}%)' for i, var in enumerate(explained_variance)],
            ax=ax3,
            vmin=-1, vmax=1  # Fixer l'échelle entre -1 et 1 pour les coefficients
        )
