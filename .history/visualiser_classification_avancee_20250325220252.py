...
def valide_avec_shapefile(classification, meta, shapefile_path):
    ...
    # Créer un sous-répertoire pour les matrices de confusion et rapports
    ...
    try:
        ...
        # Sauvegarder
        output_path = os.path.join(confusion_dir, 'matrice_confusion_pourcent.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
        ...
        # Sauvegarder
        output_path = os.path.join(confusion_dir, 'rapport_classification.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
...
def genere_carte_classification(classification, meta):
    """
    Génère une carte de classification avec légende en suivant les mêmes paramètres
    que la fonction generate_classification_map du module visualize.py
    ...
    if len(unique_classes) > 0:
        ...
        for cls in range(int(max(unique_classes)) + 1):
            if cls == 0:  # Classe non classifiée (noir)
                cmap_colors.append('#000000')  # Noir pour la classe 0
            else:
        ...
        # Créer une figure (dimensions comme dans visualize.py)
        ...
        for cls in sorted(unique_classes):
            color = COULEURS_CLASSES.get(cls, plt.cm.tab10(cls % 10))
            name = NOMS_CLASSES.get(cls, f"Classe {cls}")
        ...
        # Sauvegarder
        output_path = os.path.join(carte_dir, 'carte_classification.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
...
def genere_analyse_pca(classes_info):
    """
    Génère l'analyse PCA des classes en suivant exactement les mêmes paramètres
    que la fonction create_pca_scatterplots du module visualize.py
    ...
    # Créer un sous-répertoire pour les analyses PCA
    pca_dir = os.path.join(REPERTOIRE_SORTIE, "pca")
    ...
    try:
        ...
        # Standardiser les données (crucial pour la PCA)
        ...
        if pca_result.shape[1] >= 3:
            ...
            for class_id in np.unique(all_labels):
                ...
                ax2.scatter(
                    ...
                    c=color, 
                    alpha=0.6,
                    s=30  # Taille des points comme dans visualize.py
                )
        ...
        sns.heatmap(
            components, 
            annot=True, 
            cmap='coolwarm',  # Comme dans visualize.py
            xticklabels=columns,
        ...
        # Sauvegarder
        output_path = os.path.join(pca_dir, 'analyse_pca.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
...
def genere_scatterplots(classes_info):
    ...
    en suivant les mêmes paramètres que la fonction create_scatterplots du module visualize.py
    ...
    try:
        ...
        # Limiter à un nombre raisonnable de paires comme dans visualize.py
        if len(BANDES_SELECTIONNEES) >= 2: band_pairs.append((0, 1))
        if len(BANDES_SELECTIONNEES) >= 3: band_pairs.append((0, 2))
        if len(BANDES_SELECTIONNEES) >= 4: band_pairs.append((1, 3))
        if len(BANDES_SELECTIONNEES) >= 5: band_pairs.append((2, 4))
        if len(BANDES_SELECTIONNEES) >= 6: band_pairs.append((3, 5))
        if len(BANDES_SELECTIONNEES) >= 7: band_pairs.append((4, 6))
        if len(BANDES_SELECTIONNEES) >= 8: band_pairs.append((0, 7))
        
        # Ajouter quelques paires supplémentaires si nécessaire
        if len(BANDES_SELECTIONNEES) >= 6: band_pairs.append((1, 5))
        if len(BANDES_SELECTIONNEES) >= 7: band_pairs.append((2, 6))
        ...
        if len(BANDES_SELECTIONNEES) <= 5:
            try:
                ...
                for class_id, info in classes_info.items():
                    ...
                    for sample in data:
                        sample_dict = {f'Bande {BANDES_SELECTIONNEES[i]}': sample[i] for i in range(len(BANDES_SELECTIONNEES))}
                ...
                palette = {NOMS_CLASSES.get(cls, f"Classe {cls}"): COULEURS_CLASSES.get(cls, f"C{cls%10}") 
...
def main():
    ...
    genere_carte_classification(classification, meta)
    ...
    # Si les données originales sont disponibles, générer les analyses PCA et scatterplots
    if os.path.isfile(FICHIER_DONNEES_ORIGINALES):
        try:
            # Charger les données originales
            with rasterio.open(FICHIER_DONNEES_ORIGINALES) as src:
                # Sélectionner les bandes spécifiées
                donnees_originales = np.stack([src.read(b) for b in BANDES_SELECTIONNEES])
                print(f"Données originales chargées depuis {FICHIER_DONNEES_ORIGINALES}")
                print(f"Dimensions: {donnees_originales.shape}")
            
            # Extraire les données par classe
            classes_info = genere_donnees_par_classe(classification, donnees_originales)
            
            # Générer l'analyse PCA
            genere_analyse_pca(classes_info)
            
            # Générer les scatterplots
            genere_scatterplots(classes_info)
            
        except Exception as e:
            print(f"Erreur lors du traitement des données originales: {e}")
            import traceback
            traceback.print_exc()
    else:
        print(f"\nLe fichier de données originales {FICHIER_DONNEES_ORIGINALES} n'existe pas.")
        print("Pour générer l'analyse PCA et les scatterplots, modifiez la variable FICHIER_DONNEES_ORIGINALES.")
